...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default: "bg-purple-600 text-white shadow hover:bg-purple-800",
        destructive:
...
export interface ButtonLinkProps extends VariantProps<typeof buttonVariants> {
  ...
  text: string;
  className: string;
  target?: string; // Include target as an optional prop
}
const LinkButton = React.forwardRef<HTMLAnchorElement, ButtonLinkProps>(
  ({ variant, size, href, text, className, target, ...props }, ref) => {
    return (
      <Link
        className={classNames(buttonVariants({ variant, size, className }))}
        {...props}
